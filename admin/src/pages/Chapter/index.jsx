  ...
  const fetchChapters = useCallback(
    async (keyword = searchText) => {
      ...
      try {
        ...
        message.error(
          error.response?.data?.message || 'Không thể tải danh sách chương.'
        );
      } finally {
        setLoading(false);
      }
  ...
  const columns = [
    {
      ...
      key: 'chapterId',
      width: 90,
      sorter: (a, b) => a.chapterId - b.chapterId,
          ...
          <Space>
            <Tooltip title="Xem câu hỏi">
              <Button
                ...
                onClick={() => {
                  setChapterIdToView(record.chapterId);
            ...
            <Button
              ...
              icon={<EditOutlined />}
              onClick={() => {
                setChapterIdToUpdate(record.chapterId);
