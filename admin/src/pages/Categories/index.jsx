...
export default function CategoryManager() {
  ...
  const fetchCategories = useCallback(async (keyword = searchText) => {
    setLoading(true);
    try {
      ...
      message.error(error.response?.data?.message || "Không thể tải danh sách khoa.");
    } finally {
      setLoading(false);
    }
  }, [searchText, viewMode]);
  ...
  const handleRestore = async (categoryId) => {
    try {
      ...
      message.error(error.response?.data?.message || "Không thể khôi phục khoa.");
    }
  };
  ...
  const columns = [
    { title: "ID", dataIndex: "categoryId", key: "categoryId", width: 90, sorter: (a, b) => a.categoryId - b.categoryId },
    { title: "Tên khoa", dataIndex: "categoryName", key: "categoryName", render: (text) => <Text strong>{text}</Text> },
    {
      title: "Mô tả",
  ...
  return (
    <>
      <ManagementPageLayout
        title={<Space><AppstoreOutlined /> Quản lý khoa</Space>}
        filters={filters}
        table={<Table columns={columns} dataSource={categories} rowKey="categoryId" loading={loading} pagination={{ pageSize: 7 }} scroll={{ x: 760 }} />}
        onReload={() => fetchCategories(searchText)}
        onAdd={isAdmin && viewMode === "active" ? () => setIsAddModalOpen(true) : undefined}
      />

      <AddCategoryModal isModalOpen={isAddModalOpen} onCancel={handleCloseModal} onSuccess={() => { fetchCategories(); handleCloseModal(); }} />

      {selectedCategoryId && (
        <UpdateCategoryModal
          isModalOpen={isUpdateModalOpen}
          onCancel={handleCloseModal}
          onSuccess={() => { fetchCategories(); handleCloseModal(); }}
