...
public class GeminiAiClient implements AiClient {
    ...
    public String generateExplanation(String systemPrompt, String userPrompt) {
        ...
        if (!StringUtils.hasText(apiKey)) {
            throw new CustomApiException("AI_NOT_CONFIGURED", "Chưa cấu hình API Key cho Google Gemini.",
                    HttpStatus.SERVICE_UNAVAILABLE);
        }

        String model = StringUtils.hasText(aiProperties.getGemini().getModel()) ? aiProperties.getGemini().getModel()
                : "gemini-2.0-flash";
        String baseUrl = StringUtils.hasText(aiProperties.getGemini().getBaseUrl())
        ...
        try {
            ...
            if (e.getStatusCode().value() == 429) {
                throw new CustomApiException("AI_RATE_LIMIT",
                        "Dịch vụ Google Gemini đang hết hạn mức hoặc quá tải. Vui lòng thử lại sau.",
                        HttpStatus.TOO_MANY_REQUESTS);
            }
            throw new CustomApiException("AI_SERVICE_ERROR", "Google Gemini API phản hồi lỗi: " + e.getStatusCode(),
                    HttpStatus.BAD_GATEWAY);
        } catch (Exception e) {
