...
public class QuestionServiceImpl implements QuestionService {
    ...
    public List<QuestionDto> getPracticeQuestionsByChapter(Long chapterId, Integer limit, String difficulty, String mode, UUID userId) {
        return questionPracticeService.getPracticeQuestionsByChapter(chapterId, limit, difficulty, mode, userId);
    }

    @Override
    @Transactional(readOnly = true)
    public List<QuestionDto> getPracticeQuestionsByChapter(Long chapterId, Integer limit, String difficulty, String mode, UUID userId, boolean includeCorrectAnswers) {
        return applyCorrectAnswerVisibility(
                getPracticeQuestionsByChapter(chapterId, limit, difficulty, mode, userId),
    ...
    public List<QuestionDto> getSmartWrongPracticeQuestions(Long subjectId, Long chapterId, Integer limit, String difficulty, String strategy, UUID userId) {
        return questionPracticeService.getSmartWrongPracticeQuestions(subjectId, chapterId, limit, difficulty, strategy, userId);
    }

    @Override
    @Transactional(readOnly = true)
    public List<QuestionDto> getSmartWrongPracticeQuestions(Long subjectId, Long chapterId, Integer limit, String difficulty, String strategy, UUID userId, boolean includeCorrectAnswers) {
        return applyCorrectAnswerVisibility(
                getSmartWrongPracticeQuestions(subjectId, chapterId, limit, difficulty, strategy, userId),
                includeCorrectAnswers
        );
    }
    ...
    public List<QuestionDto> getQuestionsBySubjectAndNumber(Long subjectId, int number) {
        List<Question> questions = questionDetailLoader.loadInSameOrder(questionRepository.findRandomQuestionsBySubject(subjectId, number));
    ...
    public List<QuestionDto> getQuestionsBySubjectAndDifficulty(Long subjectId, int number, String difficulty) {
        ...
        List<Question> questions = questionDetailLoader.loadInSameOrder(
                questionRepository.findQuestionsBySubjectAndDifficulty(subjectId, difficulty, number)
        );
    ...
    public List<QuestionDto> getQuestionsByChapter(Long chapterId, int number) {
        // Lấy câu hỏi từ repository
        List<Question> questions = questionDetailLoader.loadInSameOrder(questionRepository.findQuestionsByChapter(chapterId, number));
    ...
    public List<QuestionDto> filterQuestions(String keyword, Long subjectId, Long chapterId, String difficulty, Boolean deleted, Boolean examEnabled, Boolean practiceEnabled, UUID creatorId) {
        ...
        QuestionDifficulty normalizedDifficulty = difficulty == null || difficulty.isBlank()
                ? null
    ...
    @Override
    public Page<QuestionDto> filterQuestionsPage(
            String keyword,
            Long subjectId,
            Long chapterId,
            String difficulty,
    ...
    private List<QuestionDto> applyCorrectAnswerVisibility(List<QuestionDto> questions, boolean includeCorrectAnswers) {
        if (includeCorrectAnswers || questions == null) {
            return questions;
        }
        questions.forEach(this::stripCorrectAnswer);
        return questions;
    }

    private void stripCorrectAnswer(QuestionDto question) {
    ...
    private boolean hasLockedQuestionChanges(Question existingQuestion, QuestionDto questionDto) {
        ...
        QuestionType incomingType = questionMapper.parseQuestionType(questionDto.getQuestionType());
        QuestionType existingType = existingQuestion.getQuestionType() == null
    ...
    private void validateCorrectAnswers(QuestionDto questionDto) {
        ...
        long correctAnswerCount = answers.stream()
                ...
                .count();
        QuestionType questionType = questionMapper.parseQuestionType(questionDto.getQuestionType());

        if (questionType == QuestionType.SINGLE_CHOICE && correctAnswerCount != 1) {
            throw new CustomApiException("Câu hỏi chọn một phải có đúng 1 đáp án đúng", HttpStatus.BAD_REQUEST);
        }
        if (questionType == QuestionType.MULTIPLE_CHOICE && correctAnswerCount < 2) {
    ...
    public QuestionDto restore(Long questionId) {
        ...
        Question restoredQuestion = questionDetailLoader.loadByIdsInSameOrder(List.of(questionId)).stream()
                .findFirst()
                .orElseThrow(() -> new CustomApiException("Không tìm thấy câu hỏi", HttpStatus.NOT_FOUND));
    ...
    public Map<String, Object> totalQuestionBySubject(Long subjectId) {
        ...
        for (Object[] row : counts) {
            ...
            QuestionDifficulty difficulty = (QuestionDifficulty) row[1];
