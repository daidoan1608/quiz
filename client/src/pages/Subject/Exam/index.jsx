...
export default function Exam() {
  ...
  const normalizeSelectedIndexes = (value) =>
  ...
  const handleSubmit = useCallback(async () => {
    ...
    const userAnswerDtos = Object.entries(selectedAnswers).flatMap(
        ...
        if (attempt?.userExamId) {
          ...
          (attempt.userAnswerDtos || []).forEach((userAnswer) => {
            ...
            const answerIndex = (question?.answers || []).findIndex(
              (answer) =>
                (answer.answerId || answer.optionId) === userAnswer.answerId
            );
          ...
          const remainingSeconds = Math.max(
            0,
            Number(attempt.remainingTime ?? data.duration * 60)
          );
          endTimeRef.current = Date.now() + remainingSeconds * 1000;
          setTimeLeft(remainingSeconds);
          setStartTime(fallbackStartTime);
        } else if (
          restoredDraft?.endTime &&
          ...
          setTimeLeft(
            Math.max(
              0,
              Math.floor((Number(restoredDraft.endTime) - Date.now()) / 1000)
            )
          );
  ...
  if (isLoading) {
    return (
      <div className="flex min-h-[60vh] items-center justify-center">
        {texts.loadingExam || 'Đang tải đề thi...'}
      </div>
    );
  }
  ...
  const answeredCount = questions.reduce(
    (count, question, index) =>
      isQuestionAnswered(question, selectedAnswers[index]) ? count + 1 : count,
    0
  );
          ...
          <div className="grid grid-cols-12 gap-6">
            <aside className="col-span-12 space-y-6 lg:col-span-3">
              <div className="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm dark:border-gray-700 dark:bg-gray-800">
                ...
                <p className="text-sm text-gray-500 dark:text-gray-400">
                  {texts.questionCountLabel || 'Số câu'}: {questions.length}
                </p>
              </div>
            ...
            <section className="col-span-12 flex min-h-[520px] flex-col rounded-2xl border border-gray-200 bg-white shadow-sm dark:border-gray-700 dark:bg-gray-800 lg:col-span-6">
              <div className="flex-grow p-6">
                <h3 className="pb-2 text-left text-xl font-bold leading-tight text-primary">
                  {texts.questionLabel || 'Câu'} {currentQuestionIndex + 1}
                </h3>
                <div
                ...
                {currentQuestion?.imageUrl && (
                  <div className="my-4 text-center">
                    <img
                      ...
                      alt={texts.questionIllustration || 'Minh họa câu hỏi'}
                  ...
                  {currentQuestion?.answers?.map((answer, index) => {
                    ...
                    return (
                      <label
                        ...
                        className={`flex cursor-pointer items-start rounded-xl border p-4 transition-all duration-200 hover:shadow-md ${isSelected ? 'border-primary bg-primary/10 dark:bg-primary/20' : 'border-gray-200 hover:border-primary/50 dark:border-gray-600'}`}
                      >
                        <input
                        ...
                        />
                        <span
                          className={`ml-4 text-base font-medium ${isSelected ? 'text-primary dark:text-white' : ''}`}
                          dangerouslySetInnerHTML={{
              ...
              <div className="flex items-center justify-between border-t border-gray-200 p-4 dark:border-gray-700">
                ...
                >
                  ...
                  <span className="hidden sm:inline">
                    {texts.previousQuestion || 'Câu trước'}
                  </span>
                </button>
                <button
                  onClick={() =>
                ...
                >
                  <span className="hidden sm:inline">
                    {texts.nextQuestion || 'Câu tiếp theo'}
                  </span>
                  <span className="material-symbols-outlined text-base">
                    arrow_forward
                  </span>
