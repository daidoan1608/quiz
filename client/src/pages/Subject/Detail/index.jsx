  ...
  useEffect(() => {
    ...
    const fetchSubjectDetails = async () => {
      try {
        ...
        setError(texts.loadDataError || "Có lỗi xảy ra khi tải dữ liệu. Vui lòng thử lại sau.");
      } finally {
        setIsLoading(false);
      }
    };
    ...
    fetchSubjectDetails();
  }, [subjectId, texts.subjectInfoNotFound, texts.subjectDataNotFound, texts.loadDataError]);
          ...
          <div className="grid gap-0 lg:grid-cols-[minmax(0,1fr)_22rem]">
            <div className="p-6 md:p-8">
                ...
                <div>
                  ...
                  <p className="mt-4 max-w-3xl text-base leading-relaxed text-gray-600 dark:text-gray-300">
                    {subjectData.description ||
                      texts.subjectHubDescription || "Tổng quan môn học, chương ôn tập và đề kiểm tra được gom chung trong một màn hình để bạn bắt đầu nhanh hơn."}
                  </p>
                </div>
              ...
              <div className="mt-7 grid grid-cols-2 gap-4 md:grid-cols-4">
                <StatsCard title={texts.chapterCount || "Số chương"} value={subjectData.totalChapters || chapters.length} />
                <StatsCard title={texts.questions || "Câu hỏi"} value={subjectData.totalQuestions || 0} />
                <StatsCard title={texts.examCount || "Đề kiểm tra"} value={subjectData.totalExams || exams.length} />
                <StatsCard title={texts.duration || "Thời lượng"} value={`~${estimatedHours}h`} />
              </div>
            </div>

            <div className="border-t border-gray-200 bg-gray-50 p-6 dark:border-gray-700 dark:bg-gray-900/40 lg:border-l lg:border-t-0">
              <h3 className="mb-5 text-lg font-black text-gray-950 dark:text-white">{texts.readiness || "Mức độ sẵn sàng"}</h3>
              <div className="flex items-center gap-5">
                ...
                >
                  ...
                  <div className="relative z-10 text-center">
                    ...
                    <span className="text-[10px] font-bold uppercase text-gray-500">{texts.ready || "Sẵn sàng"}</span>
                  </div>
                </div>
        ...
        <div className="grid grid-cols-1 gap-6 xl:grid-cols-[minmax(0,1fr)_24rem]">
          <section className="space-y-4">
            <div className="flex items-end justify-between gap-4">
              <div>
                <p className="text-sm font-bold uppercase tracking-wide text-primary">{texts.reviewChapters || "Chương ôn tập"}</p>
                <h2 className="text-2xl font-black text-gray-950 dark:text-white">{texts.learnByChapter || "Học theo chương"}</h2>
              </div>
              <span className="rounded-full bg-primary/10 px-3 py-1 text-sm font-bold text-primary">{chapters.length} {texts.chapters || "chương"}</span>
            </div>

            {chapters.length > 0 ? (
              chapters.map((chapter, index) => (
                <ChapterCard key={chapter.chapterId} chapter={chapter} index={index} onStart={handleChapterClick} texts={texts} />
              ))
            ) : (
              <EmptyState text={texts.noChapters || "Chưa có chương bài học nào."} texts={texts} />
            )}
          </section>

          <aside className="space-y-4">
            <div>
              <p className="text-sm font-bold uppercase tracking-wide text-primary">{texts.examLabel || "Đề thi"}</p>
              <h2 className="text-2xl font-black text-gray-950 dark:text-white">{texts.quickTest || "Kiểm tra nhanh"}</h2>
            </div>

            <div className="space-y-4 xl:sticky xl:top-24">
              {exams.length > 0 ? (
                exams.map((exam, index) => <ExamCard key={exam.examId} exam={exam} index={index} onStart={handleExamClick} inProgress={inProgressExams.has(Number(exam.examId))} texts={texts} />)
              ) : (
                <EmptyState text={texts.noExams || "Chưa có bài kiểm tra nào."} compact texts={texts} />
              )}
            </div>
          </aside>
        </div>
      ...
      <div className="flex items-start gap-4">
        ...
        <div>
          ...
          <div className="mt-2 flex flex-wrap items-center gap-3 text-sm text-gray-500 dark:text-gray-400">
            <span className="inline-flex items-center gap-1"><span className="material-symbols-outlined text-base">quiz</span>{chapter.countQuestion || 0} {texts?.questions || "câu hỏi"}</span>
            <span className={`rounded-full px-3 py-1 text-xs font-bold ${hasQuestions ? "bg-emerald-50 text-emerald-700 dark:bg-emerald-900/20 dark:text-emerald-300" : "bg-gray-100 text-gray-500 dark:bg-gray-700"}`}>{hasQuestions ? texts?.canReview || "Có thể ôn" : texts?.noQuestionsYet || "Chưa có câu hỏi"}</span>
          </div>
        </div>
      </div>
      <button disabled={!hasQuestions} onClick={() => onStart(chapter)} className={`rounded-xl px-6 py-3 text-sm font-bold transition-all ${hasQuestions ? "bg-primary text-white shadow-md hover:shadow-lg active:scale-95" : "cursor-not-allowed bg-gray-100 text-gray-400 dark:bg-gray-700"}`}>{texts?.startReview || "Bắt đầu ôn"}</button>
    </div>
  );
};

const ExamCard = ({ exam, index, onStart, inProgress, texts }) => (
  <div className={`rounded-2xl border bg-white p-5 shadow-sm transition-all hover:-translate-y-0.5 hover:shadow-md dark:bg-gray-800 ${inProgress ? "border-amber-300 dark:border-amber-700" : "border-gray-200 dark:border-gray-700"}`}>
    <div className="mb-4 flex items-start gap-3">
      ...
      <div className="min-w-0">
        <div className="flex flex-wrap items-start gap-2">
          ...
          {inProgress && <span className="rounded-full bg-amber-100 px-2 py-0.5 text-[11px] font-black text-amber-700 dark:bg-amber-900/30 dark:text-amber-300">{texts?.inProgress || "Đang làm"}</span>}
        </div>
        <p className="mt-1 line-clamp-2 text-sm leading-relaxed text-gray-500 dark:text-gray-400">{exam.description || texts?.examDescriptionFallback || "Bài kiểm tra tổng hợp kiến thức môn học."}</p>
      </div>
    </div>
    <div className="mb-4 flex flex-wrap items-center gap-3 text-sm text-gray-500 dark:text-gray-400">
      <span className="inline-flex items-center gap-1"><span className="material-symbols-outlined text-base">help</span>{exam.totalQuestions || 0} {texts?.questionUnit || "câu"}</span>
      <span className="inline-flex items-center gap-1"><span className="material-symbols-outlined text-base">timer</span>{exam.duration || 60} {texts?.minutes || "phút"}</span>
    </div>
    <button onClick={() => onStart(exam)} className={`w-full rounded-xl px-5 py-3 text-sm font-bold text-white shadow-md transition-all hover:shadow-lg active:scale-95 ${inProgress ? "bg-amber-500 hover:bg-amber-600" : "bg-primary"}`}>{inProgress ? texts?.continue || "Tiếp tục" : texts?.startExam || "Bắt đầu làm"}</button>
  </div>
);
...
const EmptyState = ({ text, compact = false, texts }) => (
  <div className={`flex flex-col items-center justify-center rounded-2xl border-2 border-dashed border-gray-200 bg-white px-4 text-center shadow-sm dark:border-gray-700 dark:bg-gray-800 ${compact ? "py-10" : "py-14"}`}>
    ...
    <h3 className="mb-1 text-lg font-bold text-gray-900 dark:text-white">{texts?.noContent || "Chưa có nội dung"}</h3>
