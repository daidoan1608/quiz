...
const Roadmap = ({ groupedExams, inProgressAttempts, learningStats, texts, onContinueAttempt }) => {
    ...
    <div className="space-y-8">
      <section>
        ...
        <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
          <StatCard icon="workspace_premium" label="Điểm trung bình" value={formatScore(learningStats?.averageScore)} />
          <StatCard icon="local_fire_department" label="Streak học tập" value={`${learningStats?.streak || 0} ngày`} />
          <StatCard icon="assignment_turned_in" label="Bài đã nộp" value={learningStats?.totalAttempts || 0} />
        </div>
      </section>
      ...
      <section className="grid grid-cols-1 gap-4 lg:grid-cols-2">
        <div className="rounded-xl border border-gray-200 bg-white p-5 shadow-sm dark:border-gray-700 dark:bg-gray-800">
          <h3 className="mb-4 flex items-center gap-2 text-lg font-bold text-gray-900 dark:text-white">
            <span className="material-symbols-outlined text-red-500">troubleshoot</span>
            Môn cần củng cố
          </h3>
              ...
              {learningStats.subjectProgress.slice(0, 4).map((subject) => (
                <div key={subject.subjectName}>
                  <div className="mb-1 flex items-center justify-between text-sm">
                    <span className="font-bold text-gray-800 dark:text-gray-100">{subject.subjectName}</span>
                    <span className="text-gray-500">{formatScore(subject.averageScore)}</span>
                  </div>
                  <div className="h-2 overflow-hidden rounded-full bg-gray-100 dark:bg-gray-700">
                    <div className={`h-full rounded-full ${subject.averageScore < 70 ? "bg-red-500" : "bg-emerald-500"}`} style={{ width: `${Math.min(100, subject.averageScore)}%` }} />
                  </div>
                </div>
              ))}
            </div>
          ) : (
            <p className="text-sm text-gray-500 dark:text-gray-400">Chưa có dữ liệu bài làm để phân tích.</p>
          )}
        </div>
        ...
        <div className="rounded-xl border border-blue-100 bg-blue-50/80 p-5 shadow-sm dark:border-blue-900/40 dark:bg-blue-900/10">
          ...
          <div className="space-y-3">
            {(learningStats?.roadmap || []).map((item, index) => (
              <div key={item} className="flex gap-3 rounded-lg bg-white p-3 text-sm text-gray-700 shadow-sm dark:bg-gray-800 dark:text-gray-200">
                <span className="flex size-7 shrink-0 items-center justify-center rounded-full bg-blue-100 text-xs font-black text-blue-700 dark:bg-blue-900/40 dark:text-blue-200">{index + 1}</span>
                <span>{item}</span>
              </div>
            ))}
          </div>
...
const StatCard = ({ icon, label, value }) => (
  <div className="rounded-xl border border-gray-200 bg-white p-5 shadow-sm dark:border-gray-700 dark:bg-gray-800">
    ...
    <p className="text-sm font-bold uppercase tracking-wide text-gray-500 dark:text-gray-400">{label}</p>
