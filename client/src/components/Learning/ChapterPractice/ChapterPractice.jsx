...
export default function RevisionChap1() {
  ...
  if (isLoading) {
    return <div className="flex min-h-[60vh] items-center justify-center">Đang tải câu hỏi ôn tập...</div>;
  }

  if (!questions.length) {
    return (
      <div className="flex min-h-[60vh] flex-col items-center justify-center gap-4 p-4 text-center">
        <p className="text-lg font-semibold text-gray-700 dark:text-gray-200">Chưa có câu hỏi ôn tập cho chương này.</p>
        <button onClick={() => navigate(-1)} className="rounded-xl bg-primary px-6 py-3 font-bold text-white shadow-md">Quay lại</button>
      </div>
    );
  }
      ...
      <main className="flex-1 p-4 sm:p-6 lg:p-8">
        <div className="mx-auto max-w-7xl">
          <nav className="mb-6 flex flex-wrap items-center gap-2 text-sm font-medium text-gray-500 dark:text-gray-400">
            ...
            <span className="material-symbols-outlined text-base">chevron_right</span>
            <span className="max-w-[260px] truncate font-bold text-gray-900 dark:text-white">{title || "Câu hỏi ôn tập"}</span>
          </nav>
          ...
          <section className="mb-8 overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-sm dark:border-gray-700 dark:bg-gray-800">
            <div className="p-6 md:p-8">
              ...
              <div className="flex flex-col gap-5 md:flex-row md:items-end md:justify-between">
                <div>
                  <h1 className="text-3xl font-black tracking-tight text-gray-950 dark:text-white md:text-4xl">{title || "Câu hỏi ôn tập"}</h1>
                  <p className="mt-3 max-w-2xl text-base leading-relaxed text-gray-600 dark:text-gray-300">
                    Hoàn thành các câu hỏi ôn tập. Bạn có thể chuyển nhanh giữa các câu bằng bảng bên phải.
                  </p>
                </div>
