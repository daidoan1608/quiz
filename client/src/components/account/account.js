...
const AccountInfo = ({ user, onChangePassword, onUploadAvatar }) => {
  ...
  const [imageUrl, setImageUrl] = useState(
    user?.avatarUrl || 'https://www.gravatar.com/avatar/00000000000000000000000000000000?d=mp'
  );
  ...
  if (!user) return <div>{texts.loadingUser || 'Đang tải thông tin người dùng...'}</div>;
    ...
    <div className="change-password-form">
      ...
      <Form onFinish={onSubmit} layout="vertical">
        <Form.Item
          ...
          rules={[{ required: true, message: texts.requiredOldPassword || 'Vui lòng nhập mật khẩu cũ!' }]}
        >
          <Input.Password placeholder={texts.oldPassword || 'Mật khẩu cũ'} size="large" />
        </Form.Item>
        <Form.Item
          name="newPassword"
          label={texts.newPassword || 'Mật khẩu mới'}
          rules={[
            { required: true, message: texts.requiredNewPassword || 'Vui lòng nhập mật khẩu mới!' },
            { min: 8, message: texts.minPasswordLength || 'Mật khẩu phải có ít nhất 8 ký tự!' },
          ]}
        >
          <Input.Password placeholder={texts.newPassword || 'Mật khẩu mới'} size="large" />
        </Form.Item>
        <Form.Item
          ...
          rules={[
            { required: true, message: texts.requiredConfirmPassword || 'Vui lòng xác nhận mật khẩu mới!' },
            ({ getFieldValue }) => ({
              validator(_, value) {
                if (!value || getFieldValue('newPassword') === value) {
                  return Promise.resolve();
                }
                return Promise.reject(texts.passwordMismatch || 'Mật khẩu xác nhận không khớp!');
              },
            }),
          ]}
        >
          <Input.Password placeholder={texts.confirmPassword || 'Xác nhận mật khẩu mới'} size="large" />
        </Form.Item>
        <Form.Item>
          <Button type="primary" htmlType="submit" block>
            {texts.confirmChangePassword || 'Xác nhận đổi mật khẩu'}
          </Button>
          <Button type="default" block onClick={onCancel} style={{ marginTop: 10 }}>
            {texts.cancel || 'Hủy'}
          </Button>
        </Form.Item>
      </Form>
...
const Account = () => {
  ...
  const handleError = (error) => {
    message.error(`${texts.error || 'Đã có lỗi xảy ra'}: ${error.message || error}`);
  ...
  const fetchData = async () => {
    ...
    if (!userId) {
      message.error(texts.noUserId || 'Không tìm thấy userId. Vui lòng đăng nhập lại!');
      navigate('/login');
      return;
    }
    try {
      ...
      if (!examData.length) message.info(texts.noExams || 'Không có bài thi nào được tìm thấy.');
    } catch (error) {
      handleError(error);
    } finally {
      setLoading(false);
                  ...
                  renderItem={(test) => (
                    <List.Item className="exam-item">
                      <div className="exam-details">
                        <p className="exam-title">{test.title || texts.unnamedExam || 'Bài thi không tên'}</p>
                        <p className="exam-score">
                          {texts.score} : {(test.userExamDto.score || 0).toFixed(2)}
                        </p>
                        ...
                        <p className="exam-time">
                          {texts.time} : {formatDateTime(test.userExamDto.startTime)} -{' '}
